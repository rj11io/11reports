...
function reportIdFromHost(host: string) {
  if (host.endsWith(`.${REPORTS_DOMAIN}`))
    return host.slice(0, -REPORTS_DOMAIN.length - 1)
...
export function proxy(request: NextRequest) {
  ...
  const internalMatch = pathname.match(
    /^\/_reports\/([0-9a-hjkmnp-tv-z]{26})(?:\/|$)/
  )
  if (
    internalMatch &&
    internalId === internalMatch[1] &&
    hasReport(internalId)
  ) {
  ...
  if (!id) {
    if (pathname.startsWith("/_reports/"))
  ...
  if (!hasReport(id))
    return secure(new NextResponse("Not found", { status: 404 }))
  if (PASSTHROUGH_PATHS.some((prefix) => pathname.startsWith(prefix)))
    return NextResponse.next()
  ...
  const routes: Record<string, string> = {
    ...
    "/_download/html": `/_reports/${id}/download/html`,
  ...
  const response = NextResponse.rewrite(url, {
    request: { headers: reportHeaders(request, id) },
  })
