...
const digest = "sha256:cd199531ac4e205a8d612295fef4ff0956c73dbcdaa3467e1a48da28e23e859f"
let server
let output = ""

function request({ host = "reports.rj11.io", pathname = "/", method = "GET" } = {}) {
  return new Promise((resolve, reject) => {
    const call = http.request(
      { hostname: "127.0.0.1", port, path: pathname, method, headers: { Host: host } },
      (response) => {
        const chunks = []
...
before(async () => {
  ...
  if (!output.includes("Ready")) throw new Error(`Server did not start:\n${output}`)
})
...
test("HEAD receipt contains exact report identity", async () => {
  const receipt = await request({ host: `${id}.reports.rj11.io`, method: "HEAD" })
...
test("sandbox content is isolated and bridge-enabled", async () => {
  ...
  assert.doesNotMatch(content.headers["content-security-policy"], /allow-same-origin/)
...
test("exposure and internal-route controls hold", async () => {
  const hiddenData = await request({ host: `${id}.reports.rj11.io`, pathname: "/_download/data" })
  assert.equal(hiddenData.status, 404)

  const markdown = await request({ host: `${id}.reports.rj11.io`, pathname: "/_download/markdown" })
...
test("unknown and reserved wildcard hosts return 404", async () => {
  assert.equal((await request({ host: "01j00000000000000000000000.reports.rj11.io" })).status, 404)
  assert.equal((await request({ host: "status.reports.rj11.io" })).status, 404)
})

test("share image is available on report hosts", async () => {
  const image = await request({ host: `${id}.reports.rj11.io`, pathname: "/og.png" })
